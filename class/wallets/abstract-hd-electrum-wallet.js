...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
  ...
  getXpub() {
    ...
    console.log("Test Network: " + child.network.bech32)
    const xpub = child.toBase58();

    console.log("Test network xpub: " + xpub)
    ...
    console.log("Test network zpub: " + this._xpub)
  ...
  createTransaction(utxos, targets, feeRate, changeAddress, sequence, skipSigning = false, masterFingerprint) {
    ...
    inputs.forEach(input => {
      ...
      if (!skipSigning) {
        ...
        keyPair = bitcoin.ECPair.fromWIF(this._getWifForAddress(input.address), TESTNET);
        keypairs[c] = keyPair;
      }
  ...
  combinePsbt(base64one, base64two) {
    const final1 = typeof base64one === 'string' ? bitcoin.Psbt.fromBase64(base64one, {network: TESTNET}) : base64one;
    const final2 = typeof base64two === 'string' ? bitcoin.Psbt.fromBase64(base64two, {network: TESTNET}) : base64two;
  ...
  static _nodeToBech32SegwitAddress(hdNode) {
    return bitcoin.payments.p2wpkh({
      pubkey: hdNode.publicKey, network: TESTNET
    }).address;
  }
  ...
  cosignPsbt(psbt) {
    const seed = this._getSeed();
    const hdRoot = HDNode.fromSeed(seed, TESTNET);
