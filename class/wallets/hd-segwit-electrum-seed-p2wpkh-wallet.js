...
export class HDSegwitElectrumSeedP2WPKHWallet extends HDSegwitBech32Wallet {
  ...
  getXpub() {
    ...
    const root = bitcoin.bip32.fromSeed(mn.mnemonicToSeedSync(this.secret, MNEMONIC_TO_SEED_OPTS), TESTNET);
  ...
  _getInternalAddressByIndex(index) {
    ...
    const node = bitcoin.bip32.fromBase58(xpub, TESTNET);
  ...
  _getExternalAddressByIndex(index) {
    ...
    const node = bitcoin.bip32.fromBase58(xpub, TESTNET);
    const address = bitcoin.payments.p2wpkh({
      pubkey: node.derive(0).derive(index).publicKey, network: TESTNET
    }).address;
